...
void play_AI()
{
	...
	if(flag != 0) // this is terminal state
	{
		...
		for (int m_tr = rl_.memory_.num_elements_ - 2; m_tr >= rl_.num_input_histories_; m_tr--)
		{
			...
			{
				// Q_next = ...;
				//world.getCell(i_old, j_old).q_[0] = world.getCell(i, j).reward_ + world.getCell(i, j).getMaxQ();
			...
			//world.getCell(i_old, j_old).q_[0] = world.getCell(i, j).reward_ + world.getCell(i, j).getMaxQ();
			Q_target = Q_next + rl_.memory_.reward_array_[m+1];
			...
			for (int i = 0; i < 100; i ++)
			{
				...
				//rl_.memory_.selected_array_[m] = Q_target;
				//output_target_temp[rl_.memory_.selected_array_[m]];
